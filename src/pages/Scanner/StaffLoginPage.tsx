...
// A viewfinder-style corner bracket, reused four times (rotated per corner) around the logo — a
// small nod to what this login actually gates access to: the QR scanner.
...
export function StaffLoginPage() {
  const { session, staffUser, loading, signIn } = useAuth()
  ...
  return (
    <div className="relative box-border flex min-h-screen items-center justify-center overflow-hidden bg-[#0a1730] px-5 py-10">
      {/* Soft ambient glows — purely decorative depth behind the card, matching the dark scanner theme. */}
      ...
      <div className="relative flex w-full max-w-[420px] flex-col items-center gap-7">
        ...
        >
          <h1 className="m-0 text-[26px] font-extrabold text-white">Staff sign in</h1>
          <p className="m-0 max-w-[300px] text-[14px] text-white/55">Sign in to scan passes and check people in at the door.</p>
        </div>
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            <div className="flex min-w-0 flex-col gap-1.5">
              <label className={FIELD_LABEL_CLASSES} htmlFor="staff-login-email">
                Email
              </label>
            ...
            <div className="flex min-w-0 flex-col gap-1.5">
              <label className={FIELD_LABEL_CLASSES} htmlFor="staff-login-password">
                Password
              </label>
              <div className="relative">
                <span className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 text-white/35">
            ...
            <label className="flex cursor-pointer items-start gap-2.5">
              ...
              <span className="flex flex-col gap-0.5">
                ...
                <span className="text-[12px] text-white/45">Recommended for shared check-in phones and tablets.</span>
              </span>
            </label>
            ...
            >
              {submitting ? 'Signing in…' : 'Sign in to scan'}
            </button>
          </form>
        </div>

        <p className="m-0 text-[13px] text-white/45 [animation:fade-in-up_500ms_ease_both]" style={{ animationDelay: '180ms' }}>
          Setting up an event instead?{' '}
          <Link to="/login" className="font-bold text-[#f4b400] no-underline hover:underline">
            Go to admin sign-in
          </Link>
        </p>
      </div>
